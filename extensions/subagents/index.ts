...
const AgentScopeSchema = StringEnum(["user", "project", "both"] as const, {
  description: "Which extra agent directories to use in addition to packaged defaults. Default: user.",
  default: "user",
});
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    label: "Subagent",
    description: [
      "Delegate tasks to packaged, user, or project subagents with isolated context.",
      "Modes: single (agent + task), parallel (tasks array), chain (sequential with {previous} placeholder).",
      'Packaged agents ship with this package; user agents come from ~/.pi/agent/agents; project agents come from .pi/agents when explicitly enabled.',
    ].join(" "),
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (params.chain && params.chain.length > 0) {
        ...
        for (let i = 0; i < params.chain.length; i++) {
          ...
          const chainUpdate = onUpdate ? (partial: AgentToolResult<SubagentDetails>) => {
    ...
    renderCall(args, theme) {
      ...
      if (args.chain && args.chain.length > 0) {
        let text = theme.fg("toolTitle", theme.bold("subagent ")) + theme.fg("accent", `chain (${args.chain.length} steps)`) + theme.fg("muted", ` [+package, ${scope}]`);
      ...
      if (args.tasks && args.tasks.length > 0) {
        let text = theme.fg("toolTitle", theme.bold("subagent ")) + theme.fg("accent", `parallel (${args.tasks.length} tasks)`) + theme.fg("muted", ` [+package, ${scope}]`);
      ...
      let text = theme.fg("toolTitle", theme.bold("subagent ")) + theme.fg("accent", agentName) + theme.fg("muted", ` [+package, ${scope}]`);
